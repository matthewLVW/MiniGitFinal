...
{
    if (Dnode->head == nullptr){return;}                            // exit function if previous commit's SLL is empty
    singlyNode *curr = OG;                                          // Otherwise; Assign a traversal pointer to start at previous commit SLL head
    singlyNode *copy = new singlyNode;                              // Dynamically allocate for a new new node to be added
    Dnode->next->head = copy;                                       // assign that node to the end of the DLL (commit DS)
    while (curr != nullptr)                                         // Traverse the previous' commit SLL
    {
        if(curr->next == nullptr)                                   // If there's only one node in that list
        {
            copy->fileName = curr->fileName;                        // copy everything over
            copy->fileVersion = curr->fileVersion;                  
            break;                                                  // break out of the loop
        }
        else                                                        // If there are several nodes
        {
            singlyNode *temp = new singlyNode;                      // dynamically allocate memory for a new node to be inserted
            temp->next = nullptr;                                   // since we are adding at the end, the next pointer will point to null
            copy->fileName = curr->fileName;                        // copy everything over
            copy->fileVersion = curr->fileVersion;                  
            copy->next = temp;                                      // new node will be inserted at the end of the list
            copy = copy->next;                                      // update copy pointer
            curr = curr->next;                                      // update traversal pointer
        }
    }
    printDS();                                                      // print the data structure
}
...
void miniGit::checkout(int commitnumber)                           // Function for check out (Matthew's implementation)
{
    ...
    singlyNode *tmp = crawler->head;                                // Declare a SLL traversal pointer and set it to start at head
    while(tmp != nullptr)                                           // Traverse the SLL   
    {                                                               // Copied will store the according file version in miniGit dir
        copied = localDirectory + "/.minigit/" + renameFile(tmp, tmp->fileName, tmp->fileVersion);
        file.open(tmp->fileName);                                   // Open the file in the main directory
        if(file.fail())                                             // if file fails to open
        {
            // cout<<"test"<<endl;
            copied = "cp " + copied + " " + localDirectory;         // Copy file into the directory
            system(copied.c_str());
        }
        else                                                        // Otherwise if file exists
        {                                                           // Remove the file                           
            string remove = "rm " + localDirectory + "/" + tmp->fileName;
            system(remove.c_str());                                 
            postcopy=copied;                                        // Store copied into a temp variable
            postcopy2=copied;                                       // Using another temp variable
            copied = "cp " + copied + " " + localDirectory;         // Copy the file
            system(copied.c_str());
            postcopy = "mv " + postcopy +" " + tmp->fileName;       // move the file
            system(postcopy.c_str());                               // since the file will be copied into main dir, we remove it
            postcopy2= "rm " + localDirectory + "/" + renameFile(tmp, tmp->fileName, tmp->fileVersion);
            system(postcopy2.c_str());                              // copy everything over
    ...
    tmp=tmp->next;                                                  // increment SLL pointer
...
//     }
// }
void miniGit::print_difference(string filename,int commitnumber)
{
    ifstream(file);                                                 // Declare an ifstream object to main dir file
    ifstream(file2);                                                // Declare an ifstream object to open miniGit file
    string filename2, word,word1;                                   // Declare string variables to use for storage                          
    file.open(filename);                                            // Open the file being read in from user input
    singlyNode* tmp = currCommit(commitnumber)->head;               // assign SLL node to start at the return node of commit num
    while(tmp != nullptr)                                           // Traverse the SLL  
    {
        if(tmp->fileName == filename)                               // Once we found the file
        {                                                           // assign the filename2 as the path to that file in miniGit
            filename2 = ".minigit/" + renameFile(tmp, tmp->fileName, tmp->fileVersion);
            file2.open(filename2.c_str());                          // Open the file
            break;
        }
        tmp=tmp->next;                                              // increment traversal pointer until we found the file
    }
    while (!file.eof())                                             // read until end of file
    {
        getline(file, word);                                        // use getline to read in each line of the main file
        getline(file2,word1);                                       // same method but for the miniGit file
        if (word != word1)                                          // if we found a difference, then we print the first difference
        {
...
bool miniGit::check_difference(string filename,int commitnumber)    // Same difference function except we use this one to help with status
{                                                                   // returns a bool value instead of a print function
    ifstream(file);                                                 // Declare an ifstream object to main dir file
    ifstream(file2);                                                // Declare an ifstream object to open miniGit file
    string filename2, word,word1;                                   // Declare string variables to use for storage                          
    file.open(filename);                                            // Open the file being read in from user input
    ...
    while (!file.eof())                                             // read until end of file
    {
        getline(file, word);                                        // use getline to read in each line of the main file
        getline(file2,word1);                                       // same method but for the miniGit file
        if (word != word1){return true;}                            // if we found a difference, return true
    }
...
{
    singlyNode* tmp = currCommit(commitnumber)->head;               // assign SLL node to start at the return node of commit num
    while(tmp != nullptr)                                           // Traverse the SLL to check for difference   
    {   
